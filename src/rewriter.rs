//! Deterministic query rewriter for HA replay correctness.
//!
//! Replaces non-deterministic Cypher function calls with parameter references
//! containing concrete values, ensuring followers replay identical results.
//!
//! Ported from graphd/strana's rewriter.rs.
...
pub fn rewrite_query(query: &str) -> RewriteResult {
    ...
    // SAFETY: input was valid UTF-8, we only replaced ASCII function calls with ASCII param refs
...
/// Respects string literals. Case-insensitive keyword matching with word boundary checks.
fn rewrite_remove(query: &str) -> String {
            ...
            if before_ok && after_ok {
                ...
                if let Some((prop_end, has_dot)) = scan_property_expr(bytes, j) {
                    if has_dot {
                        ...
                        // Handle comma-separated multi-property: REMOVE n.a, n.b
                        loop {
                            let mut k = j;
                            ...
                            if let Some((next_end, next_has_dot)) = scan_property_expr(bytes, k) {
                                if next_has_dot {
                                    output.extend_from_slice(b", ");
                                    output.extend_from_slice(&bytes[k..next_end]);
                                    output.extend_from_slice(b" = NULL");
                                    j = next_end;
                                } else {
                                    break;
    ...
    if changed {
        // SAFETY: input was valid UTF-8, we only replaced ASCII keywords
...
/// Returns `(end_pos, has_dot)` where `has_dot` indicates property access (n.prop)
/// vs label expression (n:Label) which we don't rewrite.
...
/// Scan a single Cypher identifier at `pos`.
...
/// Checks word boundaries so `xgen_random_uuid()` is NOT matched.
fn try_match_func(bytes: &[u8], pos: usize) -> Option<(usize, usize)> {
    // Word boundary: preceding char must not be alphanumeric or underscore
...
fn generate_timestamp() -> String {
    let dur = SystemTime::now().duration_since(UNIX_EPOCH).expect("system clock before epoch");
...
mod tests {
    ...
    fn test_rewrite_gen_random_uuid() {
        ...
        assert_eq!(r.query, "CREATE (:T {id: $__hakuzu_uuid_0})");
    ...
    fn test_rewrite_current_timestamp() {
        ...
        assert_eq!(r.query, "RETURN $__hakuzu_now_0 AS ts");
        assert_eq!(r.generated_params.len(), 1);
        assert_eq!(r.generated_params[0].0, "__hakuzu_now_0");
        let val = r.generated_params[0].1.as_str().unwrap();
        assert!(val.len() >= 27, "Timestamp too short: {val}");
        assert_eq!(&val[4..5], "-");
        assert_eq!(&val[10..11], "T");
        assert!(val.ends_with('Z'), "Timestamp must end with Z for UTC: {val}");
    }

    #[test]
    fn test_rewrite_current_date() {
        let r = rewrite_query("RETURN current_date() AS d");
        assert_eq!(r.query, "RETURN $__hakuzu_date_0 AS d");
        assert_eq!(r.generated_params.len(), 1);
        assert_eq!(r.generated_params[0].0, "__hakuzu_date_0");
        let val = r.generated_params[0].1.as_str().unwrap();
    ...
    fn test_rewrite_multiple_same_function() {
        ...
        assert_eq!(
            r.query,
            "CREATE (:T {a: $__hakuzu_uuid_0, b: $__hakuzu_uuid_1})"
        );
    ...
    fn test_rewrite_mixed_functions() {
        ...
        assert_eq!(
            r.query,
            "CREATE (:T {id: $__hakuzu_uuid_0, ts: $__hakuzu_now_0, d: $__hakuzu_date_0})"
        );
    ...
    fn test_case_insensitive() {
        ...
        assert_eq!(r1.query, "RETURN $__hakuzu_uuid_0");

        let r2 = rewrite_query("RETURN Gen_Random_Uuid()");
        assert_eq!(r2.query, "RETURN $__hakuzu_uuid_0");

        let r3 = rewrite_query("RETURN CURRENT_TIMESTAMP()");
        assert_eq!(r3.query, "RETURN $__hakuzu_now_0");
    ...
    fn test_remove_combined_with_function_rewrite() {
        ...
        assert_eq!(
            r.query,
            "MATCH (n) SET n.ts = NULL SET n.id = $__hakuzu_uuid_0"
        );
        assert_eq!(r.generated_params.len(), 1);
    }
    ...
    fn test_merge_params_only_generated() {
        let gen = vec![("__hakuzu_uuid_0".to_string(), serde_json::json!("abc-123"))];
        let merged = merge_params(None, &gen).unwrap();
        assert_eq!(merged["__hakuzu_uuid_0"], "abc-123");
    }
    ...
    #[test]
    fn test_no_match_with_prefix_identifier() {
        // xgen_random_uuid() should NOT be rewritten — 'x' is alphanumeric before func name
        let r = rewrite_query("RETURN xgen_random_uuid()");
        assert_eq!(r.query, "RETURN xgen_random_uuid()");
        assert!(r.generated_params.is_empty());
    }

    #[test]
    fn test_no_match_with_underscore_prefix() {
        // _gen_random_uuid() should NOT be rewritten — '_' before func name
        let r = rewrite_query("RETURN _gen_random_uuid()");
        assert_eq!(r.query, "RETURN _gen_random_uuid()");
        assert!(r.generated_params.is_empty());
    }

    #[test]
    fn test_match_after_non_word_char() {
        // After '(' or space is fine
        let r = rewrite_query("RETURN (gen_random_uuid())");
        assert_eq!(r.query, "RETURN ($__hakuzu_uuid_0)");
        assert_eq!(r.generated_params.len(), 1);
    }
    ...
    #[test]
    fn test_remove_multi_property() {
        let r = rewrite_query("MATCH (n) REMOVE n.a, n.b");
        assert_eq!(r.query, "MATCH (n) SET n.a = NULL, n.b = NULL");
    }

    #[test]
    fn test_remove_multi_property_three() {
        let r = rewrite_query("MATCH (n) REMOVE n.a, n.b, n.c");
        assert_eq!(r.query, "MATCH (n) SET n.a = NULL, n.b = NULL, n.c = NULL");
    }

    #[test]
    fn test_remove_multi_property_extra_whitespace() {
        let r = rewrite_query("MATCH (n) REMOVE n.a ,  n.b");
        assert_eq!(r.query, "MATCH (n) SET n.a = NULL, n.b = NULL");
    }

    // --- REMOVE in double quotes ---
    ...
    // --- current_date vs current_timestamp ordering ---

    #[test]
    fn test_current_date_and_timestamp_in_same_query() {
        let r = rewrite_query("RETURN current_date() AS d, current_timestamp() AS ts");
        assert_eq!(
            r.query,
            "RETURN $__hakuzu_date_0 AS d, $__hakuzu_now_0 AS ts"
        );
        assert_eq!(r.generated_params.len(), 2);
    }
